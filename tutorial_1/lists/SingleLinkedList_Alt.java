...
public class SingleLinkedList_Alt<E> implements ListADT<E> {
    ...
    public E removeBack() {
        if(isEmpty()){ // if the list is empty, we cannot remove
            ...
            return removeFront(); // if the list has only one element, the first element is the last, we can remove it
        }else{
            E element = lastNode.getElement();              	// we read the last element value before we remove it, to return it
            Node<E> newLast = findSecondToLastNode(firstNode);  // we find the second-to-last element, note that the list has for sure at least 2 elements here
