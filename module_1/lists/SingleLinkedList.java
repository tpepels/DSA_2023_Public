...
public class SingleLinkedList<E> implements ListADT<E> {
    ...
    /**
     * Support method which returns the reference of the last node in the list
     * starting from the Node currentnode, passed as argument
     * 
    ...
    private Node<E> findLastNode(Node<E> currentNode) {
        ...
        return findLastNode(currentNode.getNext()); // Recursive call: if the list has more than one element, the last
                                                    // node is in the list starting from the next node
    }
    ...
    public void insertBack(E element) {
        if (isEmpty()) { // if the list is empty, we insert in the front
            insertFront(element);
        } else { // if the list is not empty, we find the last node and append the new element to
                 // it
    ...
    public E removeBack() {
        if (isEmpty()) { // if the list is empty, we cannot remove
            System.out.println("The list is empty");
            return null;
        } else if (size == 1) { // if the list has only one element, the first element is the last, we can
                                // remove it
            return removeFront();
        } else {
            Node<E> secondToLastNode = findSecondToLastNode(firstNode); // we find the second-to-last element, note that
                                                                        // the list has for sure at least 2 elements
                                                                        // here
            E element = secondToLastNode.getNext().getElement(); // we read the last element value before we remove it,
                                                                 // to return it
