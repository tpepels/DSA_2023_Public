...
public class ArrayBasedBinaryTree<E extends Comparable<E>> implements BinaryTree<E> {
    ...
    /**
     * This recursive function move the subtree starting in the rootSubTree node in
     * the position
     * specified by the newIndex attribute.
     * 
     * It first read the currentIndex of the rootSubTree node in order to determine
     * the new indeces
     * of the children (used in the recursive calls).
     * Then, it removes the node from the current poisition in the array.
     * 
     * Then, it moves the subtrees of the children (if these exist), using
     * recursively itself. The new
     * index of each child is determined using the newIndex of the rootSubTree node.
     * 
     * Finally, it assign the rootSubTree to its newIndex, and update its myIndex
     * attribute
     */
